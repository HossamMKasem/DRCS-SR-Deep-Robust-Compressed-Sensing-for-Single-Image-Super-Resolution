...
if opt.eval then
  ...
  while opt.e == -1 or opt.e >= epoch do
    ...
    if opt.output ~= '' then
     torch.save('/data/hossamkasem/13-CS/2-Trained_Network/1-Mnist/1-Transformation /1-R/1-50/2-STN/Saved_network/Trained_Network_'..epoch..'.bin', network)
      torch.save(opt.output.."norm", {mean, std})
	  torch.save('/data/hossamkasem/13-CS/2-Trained_Network/1-Mnist/1-Transformation /1-R/1-50/2-STN/Saved_network/accuracy_'..epoch..'.bin', accuracy)
    end
    epoch = epoch + 1
  end
