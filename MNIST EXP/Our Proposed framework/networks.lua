...
function networks.new_conv_CS1(nbr_input_channels,nbr_output_channels,
                           multiscale, no_cnorm, filter_size)
  multiscale = multiscale or false
  no_cnorm = no_cnorm or false
  filter_size = filter_size or 3
...
function networks.new_conv_CS2(nbr_input_channels,nbr_output_channels,
                           multiscale, no_cnorm, filter_size)
  multiscale = multiscale or false
  no_cnorm = no_cnorm or false
  filter_size = filter_size or 3
...
function networks.network_resnet(Number_RB, no_cuda)
										 
 
  local main_network = nn.Sequential()
  local RB=nn.Sequential()
  local net_2=nn.Sequential()
  branch_2=nn.Identity()
  branch_1=nn.Sequential()
  branch_1:add(networks.new_conv_CS1(128, 128,
                                           	 false, false, 3))
  branch_1:add(networks.new_conv_CS2(128, 128,
                                           	 false, false, 3))
   local net_1=nn.ConcatTable()
    net_1:add(branch_1)
    net_1:add(branch_2)
    net_2:add(net_1)
    net_2:add(nn.CAddTable())
    RB:add(net_2)
    RB:add(nn.ReLU())

for i=1, Number_RB do
main_network:add(RB)
end	

main_network:add(networks.new_conv_CS2(128, 64,
                                           	 false, false, 3))
main_network:add(networks.new_conv_CS2(64, 32,
                                           	 false, false, 3))	
main_network:add(networks.new_conv_CS2(32, 16,
                                           	 false, false, 3))												 

main_network:add(networks.new_conv_CS2(16, 1,
                                           	 false, false, 3))
...
function networks.new(opt)
  ...
  elseif opt.cnn then
    ...
    local conv1 = networks.new_conv(1, nbr_elements[1],
                                    false, opt.no_cnorm)
    ...
    network:add(networks.new_spatial_tranformer_new(opt.locnet,
                                                  ...
                                                  nil, nil,
                                                  opt.no_cuda))
	conv_512_128=networks.new_conv2(512, 128,
                                           	 false, false, 3)	
											 
    network:add(conv_512_128)
	network :add(networks.network_resnet(opt.Number_RB,opt.no_cuda))
    network:add(conv1)
    network:add(conv2)
    network:add(fc)
    network:add(classifier)
	torch.save('sample_network.bin',network)
  else
