...
function networks.new_spatial_tranformer(locnet, rot, sca, tra,
  ...
  branch2:add(nn.AffineGridGeneratorBHWD(2*input_size,2*input_size))
  if not no_cuda then -- see (1) below
...
network = nn.Sequential()
  ...
  network:add(nn.SpatialConvolution(3, 64, 9, 9, 1, 1, 2, 2))  
  --network:add(nn.SpatialBatchNormalization(64))
  network:add(nn.ReLU(true))

 network:add(nn.SpatialConvolution(64, 32, 1, 1, 1, 1, 2, 2))  
  network:add(nn.ReLU(true))
  network:add(nn.SpatialConvolution(32, 3, 5, 5, 1, 1, 2, 2))
